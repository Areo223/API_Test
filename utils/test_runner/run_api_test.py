...
# from utils.test_runner.run_test_runner import RunTestRunner
# from apps.api_test.model_factory import ApiReportCase as ReportCase,ApiReportStep as reportStep,ApiStep as Step
#
#
...
#
#     Attributes:
#         api_id_list (list): 要执行的接口id列表
#         report_id (int): 报告id
#         env_code (str): 环境代码
#         env_name (str): 环境名称
#         project (dict): 解析后的当前服务信息
#         count_step (int): 步骤计数器
#
#     Methods:
#         __init__(self, api_id_list, report_id, env_code, env_name): 初始化类实例
#         parse_and_run(self): 异步解析并运行接口
#         format_data_for_template(self): 格式化数据以适配模板
#         get_format_api(self, project, api_id): 获取格式化后的接口数据
#         run_test_data(self): 获取运行测试数据
#         init_parsed_data(self): 初始化解析后的数据
#     """
#
